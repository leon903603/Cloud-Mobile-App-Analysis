...
```
       ...
       └──────────────────────────┬───────────────────────────────┘
                                  │
                                  ▼ (透過 Tailscale <TAILSCALE_HOST2_IP>:5001 加密穿透)
       ┌──────────────────────────────────────────────────────────┐
       │ 🖥️ 主機二：實驗室舊伺服器 (192.168.50.x, ubuntu-2004)    │
       │                                                          │
       │  [線上生產環境 (Production)]                              │
       ...
       ┌──────────────────────────────────────────────────────────┐
       │ 🚀 主機三：實驗室 5860 怪物主機 (192.168.50.x, Precision)│
...
- **雲端沙箱環境**：
  - 規格：AWS Sydney EC2 `c6g.metal` ARM64 裸機實體機（Instance ID: `i-xxxxxxxxx`）。
...
- **執行個體**：`i-xxxxxxxxx` (`t3.small`)，地區：亞太雪梨 `ap-southeast-2`
- **內網 IP (AWS 內部)**：`172.31.x.x`
- **Tailscale IP**：`<TAILSCALE_HOST1_IP>`
- **SSH 帳號**：`ubuntu`
- **專案目錄**：
...
- **核心設定檔**：`~/CMAA-Int/.env`
  ...
  - `ANDROID_STATIC_API=http://<TAILSCALE_HOST2_IP>:5001/analyze_apk`（透過 Tailscale 直通主機二 53 線上生產節點）
- **外部雲端相依**：
...
- **內網 IP**：`192.168.50.x` (主機名 `ubuntu-2004`)
- **Tailscale IP**：`<TAILSCALE_HOST2_IP>`
- **SSH 帳號 / 密碼**：`islab` / `[REDACTED_PASSWORD]`
...
- **運作中的微服務**：
  - **PostgreSQL 16**：`localhost:5433` (帳密 `crawler`/`[REDACTED_PASSWORD]`, DB `appcrawler`)，**已登記 1,211,081 筆 APP**
